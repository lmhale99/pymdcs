...
def database_info(name):
    ...
    ----------
    name : str
        The name assigned to the database to get values for.
    Returns
    -------
    dict
...
def database_load(name):
    """
    Loads saved database system information based on the database's assigned
    ...
    -------
    MDCS
        An MDCS object with the loaded database access information.
    """
    accessfile = os.path.join(os.path.dirname(os.path.abspath(__file__)), '.mdcs')
    if os.path.isfile(accessfile):
        with open(accessfile) as fp:
...
class MDCS(object):
    ...
    def curate(self, content, title, template, validate=True):
        ...
        # Read content in if a file name is given
        try:
